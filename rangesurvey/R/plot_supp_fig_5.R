...
plot_supp_fig_5 <- function(data, config) {
  ...
  postcode_areas_shapefile <- rgdal::readOGR(dsn = dir,
                                  layer = file)
  
  results <- data %>%
    dplyr::mutate(postcode_area = sapply(postcode,
                                         FUN = function(X) {
  ...
  my.palette <- RColorBrewer::brewer.pal(n = 9, name = "Purples")
  
  # I can add more tones to this palette :
  coul = colorRampPalette(my.palette)(max(results$n))
