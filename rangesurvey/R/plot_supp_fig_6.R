...
plot_supp_fig_6 <- function(speciesdata = survey_responses_to_model){
  ...
  data<- speciesdata %>% 
    dplyr::select(species, attitude_to_species) %>% 
    na.omit() %>% 
    dplyr::group_by(., species, attitude_to_species) %>%
    dplyr::count()%>%
    dplyr::ungroup() %>% 
    tidyr::pivot_wider(species,
                       attitude_to_species,
                       values_from = n,
                       values_fill  = 0) %>% 
    dplyr::mutate(species = sapply(species,stringr::str_replace, pattern = "(^.)",replacement = "\\1. "))%>% 
...
species_attitude_plot <-HH::likert(data,
       ...
       main = "Attitude across Species", xlim = c(-100, 100), as.percent = T,
       positive.order = TRUE,
