...
rename_VoN <- function(data) {
  ...
  von_perceptions <- von_numeric %>%
    dplyr::ungroup() %>%
    dplyr::mutate(
      Steward = apply(dplyr::select(.data$., dplyr::starts_with("steward")), 1, mean, na.rm = T),
      Partner = apply(dplyr::select(.data$., dplyr::starts_with("partner")), 1, mean, na.rm = T),
      Participant = apply(dplyr::select(.data$., dplyr::starts_with("participant")), 1, mean, na.rm = T),
      Master = apply(dplyr::select(.data$., dplyr::starts_with("master")), 1, mean, na.rm = T)
    )
